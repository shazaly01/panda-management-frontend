<!--src\views\sales\components\SaleFormActions.vue-->
<template>
  <div class="space-y-6 mt-8">
    ...
    >
      ...
      <div>
        <AppInput
          ...
          class="text-xl font-bold text-green-600 focus-within:ring-green-500/20"
          placeholder="0.00"
        />
      </div>
    ...
    <div
      ...
      class="bg-indigo-50/50 dark:bg-indigo-900/10 p-5 rounded-xl border border-indigo-100 dark:border-indigo-800/50 animate-fade-in-down"
    >
      <div class="w-full md:w-1/2">
        <TreasuriesDropdown
          id="treasury-id"
          label="أين سيتم إيداع المبلغ المدفوع؟ (الخزينة/البنك)"
          v-model="form.treasury_id"
          :required="true"
        />
        <p v-if="errors?.treasury_id" class="text-rose-500 text-[10px] font-bold mt-1 mr-2">
          بما أنه تم استلام مبلغ، يجب تحديد الخزينة *
        </p>
      </div>
    </div>
    ...
    >
      ...
      <AppButton type="submit" :disabled="isSaving" class="px-8 shadow-md w-full md:w-auto">
...
// استخدام defineModel لربط الـ form الممرر من الأب وتجنب خطأ Mutation
const form = defineModel('form', { type: Object, required: true })

// استقبال باقي الخصائص للقراءة فقط
...
// حساب المبلغ المتبقي بشكل حي (Live Calculation)
const remainingBalance = computed(() => {
  const total = Number(props.grandTotal) || 0
  const paid = Number(form.value.paid_amount) || 0

  // نمنع ظهور قيمة سالبة إذا تم إدخال مبلغ أكبر من الفاتورة بالخطأ
  return Math.max(0, total - paid)
})
</script>

<style scoped>
/* تأثير حركة بسيط لظهور حقل الخزينة بسلاسة */
